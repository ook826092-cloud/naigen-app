# NaiGen — Android API テキストから画像生成クライアント
...
API テキストから画像生成チュートリアル (v7.12) に基づくネイティブ Android Kotlin クライアント。**バックエンドは完全に Android ネイティブ技術** (OkHttp + Coroutines + Room + DataStore) で実装され、Python や外部サーバーに依存しません。
...
|---|---|---|
| §5.1 API 概要 | `https://API 服务器` Job 非同期モード | `NaiApiClient` + `NaiRepository.generate()` |
...
トークンが間違っているか期限切れ。API プロバイダーから新しいものを取得してください。
...
MIT — [LICENSE](LICENSE) を参照。API サービスおよび購入リンクはサードパーティです; それぞれの利用規約に従ってください。
