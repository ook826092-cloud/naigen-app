...
/**
 * 风格统一访问入口：合并内置 7 + 社区 29 = 36 个静态风格。
 * 自定义风格通过 [mergedWith] 合并进来（用户加的自定义风格）。
 */
object StyleRegistry {
    ...
    /**
     * 根据 key 取风格预设（只查内置+社区，不查自定义）。
     * 如要查自定义，用 [mergedWith] 后再 get。
     */
    ...
    /**
     * 把自定义风格合并进来。返回的 map 包含内置 + 社区 + 自定义。
     * ViewModel 应该订阅这个方法的结果作为 UI 数据源。
     */
    fun mergedWith(custom: List<StylePreset>): Map<String, StylePreset> {
        val merged = HashMap(MAP)
        custom.forEach { merged[it.key] = it }
        return merged
    }
}
