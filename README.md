...
最新版本：[Releases 页面](https://github.com/ook826092-cloud/naigen-app/releases/latest)

每次 push 到 main 分支会自动触发 GitHub Actions 工作流：
1. 自增 PATCH 版本号（v2.0.0 → v2.0.1 → ...）并 commit 回仓库
2. 编译 Release APK（用 keystore 签名）
3. 自动创建 GitHub Release，附带 APK + SHA256 校验文件
...
└── app/
    ...
    └── src/main/
        ...
        │   └── util/
        │       ├── keepalive/      # ManufacturerHelper（厂商识别 + Intent 跳转）
        │       ├── ImageSaver.kt   # 降采样解码 + MediaStore 相册保存
        │       ├── DateUtils.kt
        │       └── ShareUtils.kt
        └── res/
...
# 产物：app/build/outputs/apk/release/app-release.apk（用 keystore 签名）
```

或 Android Studio → `Build` → `Generate Signed Bundle / APK` → APK → 选 keystore。

如需本地调试，可用 `./gradlew assembleDebug`（debug 签名，仅本地）。
...
清理项目：`./gradlew clean` → `./gradlew assembleRelease`。

## 版本

- v2.0.x (2026-07-15) — 当前版本，对齐教程 7.12 版本。完整变更见 [Releases](https://github.com/ook826092-cloud/naigen-app/releases)
