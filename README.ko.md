# NaiGen — Android API 텍스트-이미지 생성 클라이언트
...
API 텍스트-이미지 생성 튜토리얼 (v7.12) 기반 네이티브 Android Kotlin 클라이언트. **백엔드는 완전히 Android 네이티브 기술** (OkHttp + Coroutines + Room + DataStore)로 구현되었으며, Python이나 외부 서버에 의존하지 않습니다.
...
|---|---|---|
| §5.1 API 개요 | `https://API 服务器` Job 비동기 모드 | `NaiApiClient` + `NaiRepository.generate()` |
...
토큰이 잘못되었거나 만료됨. API 제공자로부터 새 것을 받으세요.
...
MIT — [LICENSE](LICENSE) 참조. API 서비스 및 구매 링크는 서드파티입니다; 각각의 이용약관을 따르세요.
